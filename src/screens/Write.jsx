...
export default function WriteScreen({ author, token, onPublish, onLogout }) {
    ...
    const scheduleSave = useCallback(() => {
        ...
        autoSaveTimer.current = setTimeout(() => {
            saveCurrentDraft({ title, story, country, language, photoNames: files.map(p => p.name) });
        }, 1000);
    }, [title, story, country, language, files]);

    useEffect(() => {
        scheduleSave();
        return () => { if (autoSaveTimer.current) clearTimeout(autoSaveTimer.current); };
    }, [title, story, country, language, files, scheduleSave]);
    ...
    const handlePublish = async () => {
        ...
        try {
            ...
            const post = await createDraftPost(
                { title: title.trim(), story: story.trim(), media: uploadedMedia, featuredImageId, country, language },
                token
            );
        ...
        <div style={{ minHeight: '100vh', minHeight: '100dvh', background: '#fff' }}>
            <main role="main" aria-label="Write your story" style={{ maxWidth: 560, margin: '0 auto', padding: '16px 20px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, paddingBottom: 12, borderBottom: '1px solid #e5e5e5' }}>
                    <div>
                        <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '0.85rem', color: '#888', margin: 0 }}>Writing as</p>
                        <p style={{ fontFamily: "'Source Serif 4', Georgia, serif", fontSize: '1.05rem', fontWeight: 700, color: '#1a1a1a', margin: '2px 0 0 0' }}>{author.name}</p>
                    </div>
                    <button onClick={onLogout} aria-label="Sign out" style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '0.85rem', color: '#666', background: 'none', border: '1px solid #ddd', borderRadius: 8, padding: '6px 14px', cursor: 'pointer', minHeight: 40 }}>Sign Out</button>
                </div>

                {/* Error */}
                {error && (
                    <div role="alert" style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 10, padding: '12px 16px', marginBottom: 20 }}>
                        <p style={{ color: '#dc2626', fontSize: '0.9rem', fontFamily: "'DM Sans', sans-serif", margin: 0, lineHeight: 1.5 }}>{error}</p>
                    </div>
                )}
                ...
                <div style={{ marginBottom: 20 }}>
                    <label htmlFor="story-title" style={{ display: 'block', fontFamily: "'DM Sans', sans-serif", fontSize: '0.9rem', fontWeight: 600, color: '#333', marginBottom: 6 }}>
                        Title of Your Story <span aria-hidden="true">*</span><span className="sr-only">(required)</span>
                    </label>
                ...
                <div style={{ marginBottom: 20 }}>
                    <label htmlFor="story-body" style={{ display: 'block', fontFamily: "'DM Sans', sans-serif", fontSize: '0.9rem', fontWeight: 600, color: '#333', marginBottom: 6 }}>
                        Your Story <span aria-hidden="true">*</span><span className="sr-only">(required)</span>
                    </label>
                ...
                <div style={{ marginBottom: 24 }}>
                                ...
                                <div key={i} role="listitem" style={{ position: 'relative', width: 80, height: 80 }}>
                                    {f.isVideo ? (
                                        <div style={{ width: 80, height: 80, borderRadius: 8, border: '1px solid #ddd', background: '#f3f4f6', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 2 }} aria-label={`Video: ${f.name}`}>
                                            ...
                                            <span style={{ fontSize: '0.6rem', color: '#666', fontFamily: "'DM Sans', sans-serif", maxWidth: 70, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', textAlign: 'center' }}>{formatSize(f.size)}</span>
                                        </div>
                                    ) : (
                                    ...
                                    {!f.isVideo && (
                                        <button onClick={() => toggleFeatured(i)} aria-label={f.isFeatured ? `Photo ${i + 1} is the cover photo` : `Set photo ${i + 1} as cover photo`} title={f.isFeatured ? 'Cover photo' : 'Set as cover photo'}
                                            style={{ position: 'absolute', bottom: -4, left: -4, width: 24, height: 24, borderRadius: '50%', background: f.isFeatured ? '#f59e0b' : '#e5e7eb', color: f.isFeatured ? '#fff' : '#9ca3af', border: 'none', fontSize: 12, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 24, padding: 0, lineHeight: 1 }}>★</button>
                                    )}
                                    <button onClick={() => removeFile(i)} aria-label={`Remove ${f.isVideo ? 'video' : 'photo'} ${i + 1}`}
                                        style={{ position: 'absolute', top: -6, right: -6, width: 26, height: 26, borderRadius: '50%', background: '#dc2626', color: '#fff', border: 'none', fontSize: 12, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 26, padding: 0 }}>✕</button>
                                </div>
                            ))}
                        </div>
                    )}
                    ...
                    {files.length > 0 && imageCount > 1 && (
                        <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '0.75rem', color: '#888', margin: '0 0 12px 0' }}>
                            <span style={{ color: '#f59e0b' }}>★</span> Tap the star to choose your cover photo — it appears on the Stories page.
                        </p>
                    )}
                ...
                <div style={{ marginBottom: 28, padding: '16px', background: '#faf9f7', borderRadius: 12, border: '1px solid #e8e4de' }}>
                    <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '0.85rem', fontWeight: 600, color: '#555', margin: '0 0 4px 0' }}>Optional — helps us categorise your story faster</p>
                    <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '0.75rem', color: '#888', margin: '0 0 14px 0' }}>The coordinator can also set these later.</p>
                    <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                        <div style={{ flex: '1 1 200px' }}>
                            <label htmlFor="story-country" style={{ display: 'block', fontFamily: "'DM Sans', sans-serif", fontSize: '0.8rem', fontWeight: 600, color: '#555', marginBottom: 4 }}>Your Country</label>
                            <input
                                id="story-country"
                                type="text"
                                value={country}
                                onChange={(e) => setCountry(e.target.value)}
                                placeholder="e.g. Kenya, India, Malawi…"
                ...
                <button onClick={handlePublish} disabled={!canPublish || publishing} aria-label={publishing ? publishStep || 'Submitting your story' : 'Submit your story'}
                    ...
                    {publishing ? (
                        <><span className="spinner" aria-hidden="true"></span><span aria-live="polite">{publishStep || 'Submitting…'}</span></>
                    ) : (
                        '✅ Submit My Story'
                    )}
                </button>
                ...
                {!canPublish && !publishing && (
                    <p style={{ textAlign: 'center', fontFamily: "'DM Sans', sans-serif", fontSize: '0.8rem', color: '#aaa' }}>Add a title and story to publish</p>
                )}
