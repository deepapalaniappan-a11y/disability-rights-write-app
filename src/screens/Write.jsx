...
/**
 * Generate a preview for a file — image thumbnail or video icon
 */
function createFilePreview(file) {
  if (isImage(file)) {
    return { preview: URL.createObjectURL(file), isVideo: false };
  }
  // For videos, no thumbnail — use a placeholder
...
export default function WriteScreen({ author, token, onPublish, onLogout }) {
  ...
  const [files, setFiles] = useState([]); // { file, preview, isVideo, name, size, isFeatured }
  ...
  const handleFiles = (e) => {
    ...
    setFiles((prev) => {
      ...
      // If no featured image yet and we just added an image, auto-select the first image
  ...
  const removeFile = (index) => {
    setFiles((prev) => {
      ...
      // If removed file was featured, auto-select next image
  ...
  // Publish flow: compress images → upload all media → create draft post
  const handlePublish = async () => {
    ...
    try {
      ...
      // Step 1: Compress images and prepare all files
      if (files.length > 0) {
      ...
      if (!isOnline() || err.message.includes('fetch') || err.message.includes('network')) {
        const saved = saveOfflineDraft({ title: title.trim(), story: story.trim() });
        if (saved) {
          setError('Connection lost. Your story text has been saved and will be published when you are back online. Photos and videos will need to be re-added.');
        } else {
  ...
  return (
    <div style={{
      ...
      >
        ...
        <div style={{ marginBottom: 24 }}>
              ...
              {files.map((f, i) => (
                ...
                >
                  ...
                  {f.isVideo ? (
                    ...
                    <img
                      ...
                      style={{
                        ...
                        objectFit: 'cover',
                        borderRadius: 8,
                        border: f.isFeatured ? '3px solid #f59e0b' : '1px solid #ddd'
                      }}
                    />
                  )}

                  {/* Featured star (images only) */}
                  {!f.isVideo && (
                    <button
                      onClick={() => toggleFeatured(i)}
                      aria-label={f.isFeatured ? `Photo ${i + 1} is the cover photo` : `Set photo ${i + 1} as cover photo`}
                      title={f.isFeatured ? 'Cover photo' : 'Set as cover photo'}
                      style={{
                        position: 'absolute',
                        bottom: -4,
                        left: -4,
                        ...
                        height: 24,
                        borderRadius: '50%',
                        background: f.isFeatured ? '#f59e0b' : '#e5e7eb',
                        color: f.isFeatured ? '#fff' : '#9ca3af',
                        border: 'none',
                        fontSize: 12,
                        cursor: 'pointer',
                        display: 'flex',
                  ...
                  <button
                    ...
                    aria-label={`Remove ${f.isVideo ? 'video' : 'photo'} ${i + 1}`}
                    style={{
                      position: 'absolute',
                      top: -6,
                      right: -6,
          ...
          <button
            ...
            style={{
              ...
              fontSize: '0.95rem',
              fontWeight: 500,
              color: files.length >= MAX_FILES ? '#aaa' : '#555',
              background: '#f5f5f5',
        ...
        {/* Region & Language — optional metadata */}
        <div style={{
          marginBottom: 28,
          padding: '16px',
          background: '#faf9f7',
          ...
          <p style={{
            ...
            Optional — helps us categorise your story faster
          </p>
          <p style={{
            fontFamily: "'DM Sans', sans-serif",
            fontSize: '0.75rem',
            ...
            The coordinator can also set these later.
          </p>
        ...
        <button
          ...
          style={{
            ...
            fontWeight: 700,
            fontFamily: "'DM Sans', sans-serif",
            background: canPublish && !publishing ? '#16a34a' : '#ccc',
            color: '#fff',
            border: 'none',
            borderRadius: 12,
        ...
        >
          {publishing ? (
            <>
              ...
              <span aria-live="polite">{publishStep || 'Publishing…'}</span>
            </>
          ) : (
            'Publish My Story'
          )}
